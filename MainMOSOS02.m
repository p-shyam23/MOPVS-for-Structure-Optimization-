...
UB = ones(1, D).*41; % UB - Vector of maximum possible values for decision variables.
GEN = 167*2;  % Set the maximum number of generation (GEN)
ecosize = 50;  % Set the population size (NP)
ishow = 10;

narchive=50;    % no of pareto archive   

Fobj='p10bar';% objective function
...
chromosome = initialize_variables(Fobj,ecosize, M, D, LB, UB);
...
Population = replace_chromosome(intermediate_chromosome, M,D,ecosize);
...
Pareto = MOSOS02(Fobj,D,M,LB,UB,Population,ecosize,GEN,ishow,narchive);
Pareto_MOSOS02{i}=Pareto;
save('MOSOS02_10bar.mat','Pareto_MOSOS02')
% save Pareto_SOS.txt Pareto -ascii;  % save data for future use
% %% Plot data
% if M == 2
