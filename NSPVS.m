...
f = z();
function f  = PVS_algorithm(chromosome, M, V, mu, mum, l_limit, u_limit)
...
%     R=rand;
    ...
    r(1) = i;
    while true;
        r(2) = round(N * rand + 0.5);
        if (r(2) ~= r(1)), break, end
    end
    while true
        r(3) = round(N* rand + 0.5);
        if (r(3) ~= r(1)) && (r(3) ~= r(2)), break, end
    end
    r2=r; % temp save r in r2
    r=(norm_factor/ecosize_size)*r; %normalization eq 16 
    r1=r;
    ...
    parent_r1=chromosome(r2(1),:); 
    parent_r2=chromosome(r2(2),:); 
    parent_r3=chromosome(r2(3),:); 
    ...
    for j = 1 : V
        if v1(1)>v1(3) %step 6
           if (y-y1)>x1;
               ...
               % eq 20 
               child(j)=parent_r1(j)+con1*rand*(parent_r1(j)-parent_r3(j));
           else
               % eq 21 
               child(j)=parent_r1(j)+rand*(parent_r1(j)-parent_r2(j));
           end
        else
           % eq 22 
           child(j)=parent_r1(j)+rand*(parent_r3(j)-parent_r1(j));
        end 
%        if R<=0.3333  
%             if rank(i)<rank(p2)
%                 if rand < 0.5
...
for i = 1 : gen
    mu = 20;
    mum = 20;
    offspring_chromosome = PVS_algorithm(chromosome,M, V, mu, mum, min_range, max_range);
...
Pareto_MOPVS{ii}=chromosome;
save('Pareto_MOPVS10bar_NHTSPVS.mat','Pareto_MOPVS')
     end
% if M == 2
